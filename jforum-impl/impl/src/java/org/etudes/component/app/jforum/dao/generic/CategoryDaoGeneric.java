...
public abstract class CategoryDaoGeneric implements CategoryDao
{
		...
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				try
				{
                    ...
                    // LAULIMA-107
                    Boolean duplicatesFixed = checkAndFixDuplicateCategoryDisplayOrders(selectAllByCourse(category.getContext()));
                    							
					List<Category> categories = selectAllByCourse(category.getContext());
                    
                    // checkAndFixDuplicateCategoryDisplayOrders() changed the display_order of all categories in the site
                    // get the updated display_orders for curOrder and newOrder
                    if (duplicatesFixed) {    
                        for (int i = 0; i < categories.size(); i++) {
                            if (categories.get(i).getId() == category.getId()) {
                                curOrder = categories.get(i).getOrder();
                                break;
                            }
                        }
                        for (int i = 0; i < categories.size(); i++) {
                            if (categories.get(i).getId() == related.getId()) {
                                newOrder = categories.get(i).getOrder();
                                break;
                            }
                        }
                    } // LAULIMA-107
    ...
    // LAULIMA-107
    public Boolean checkAndFixDuplicateCategoryDisplayOrders(List<Category> categories) {
        // double check that there really are duplicates
        if (siteContainsDuplicateCategoryDisplayOrders(categories)) {
        System.out.println("checkAndFixDuplicateCategoryDisplayOrders.  Duplicates found.  New maxorder + 1: " + (getMaxDisplayOrder() + 1));
            //System.out.println("there are duplicates");
            
            // we don't know if there are duplicates, triplicates, or more
            // so just reset the forum_order for ALL items
            for (final Category f : categories) {
                f.setOrder(getMaxDisplayOrder() + 1);
				...
				this.sqlService.transact(new Runnable() {
					public void run() {
						try {
							...
							sql.append("UPDATE jforum_categories SET display_order = ? WHERE categories_id = ?");							
							...
							try {
								...
								throw new RuntimeException("fix category order duplicates: dbWrite failed");
							}
							...
							// remove any external entries like entry in grade book if forum is gradable if created
							throw new RuntimeException("Error while updating category order.", e);
						}
					}
				}, "fixDuplicates: " + f.getTitle());   
            }
            return true;
        }
        return false;
    ...
    // LAULIMA-107
    protected Boolean siteContainsDuplicateCategoryDisplayOrders(List<Category> categories) {
        if (categories != null && !categories.isEmpty()) {
            for (int i = 0; i < categories.size(); i++) {
                for (int j = i + 1; j < categories.size(); j++) {
                    if (categories.get(i).getOrder() == categories.get(j).getOrder()) {
                        return true;
                    }
                }
            }
