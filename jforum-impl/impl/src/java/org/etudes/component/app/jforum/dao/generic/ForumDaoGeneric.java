...
public abstract class ForumDaoGeneric implements ForumDao
{
		...
		this.sqlService.transact(new Runnable()
		{
			public void run()
			{
				try
				{
                    ...
                    // LAULIMA-107
                    Boolean duplicatesFixed = checkAndFixDuplicateForumOrders(selectByCategoryId(forum.getCategoryId()));
                    
                    List<Forum> forums = selectByCategoryId(forum.getCategoryId());
                    
                    // checkAndFixDuplicateForumOrders() changed the forum_order of all forums in the category
                    // get the updated forum_orders for curOrder and newOrder
                    if (duplicatesFixed) {    
                        for (int i = 0; i < forums.size(); i++) {
                            if (forums.get(i).getId() == forum.getId()) {
                                curOrder = forums.get(i).getOrder();
                                break;
                            }
                        }
                        for (int i = 0; i < forums.size(); i++) {
                            if (forums.get(i).getId() == related.getId()) {
                                newOrder = forums.get(i).getOrder();
                                break;
                            }
                        }
                    } // LAULIMA-107                   
    ...
    protected Boolean checkAndFixDuplicateForumOrders(List<Forum> forums) {
        
        // double check that there really are duplicates
        if (categoryContainsDuplicateForumOrders(forums)) {
				...
				this.sqlService.transact(new Runnable() {
					public void run() {
						try {
							...
							sql.append("UPDATE jforum_forums SET forum_order = ? WHERE forum_id = ?");							
							...
							try {
								...
								throw new RuntimeException("fix Forum order duplicates: dbWrite failed");
							}
							...
							// remove any external entries like entry in grade book if forum is gradable if created
							throw new RuntimeException("Error while fixig forum duplicate order.", e);
						}
					}
				}, "fixDuplicates: " + f.getName());   
            }
            return true;
        }
        return false;
    ...
    protected Boolean categoryContainsDuplicateForumOrders(List<Forum> forums) {
        if (forums != null && !forums.isEmpty()) {
            for (int i = 0; i < forums.size(); i++) {
                for (int j = i + 1; j < forums.size(); j++) {
                    if (forums.get(i).getOrder() == forums.get(j).getOrder()) {
                        return true;
                    }
                }
            }
